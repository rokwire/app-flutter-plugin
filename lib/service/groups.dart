...
class Groups with Service implements NotificationsListener {
  ...
  Future<List<Group>?> loadGroups({GroupsContentType? contentType, String? title, String? category, Map<String, dynamic>? attributes, Set<String>? tags, int? offset, int? limit}) async {
    if (contentType == GroupsContentType.my) {
      await _updateUserGroupsFromNetSync();
      return userGroups;
    } else {
      return await _loadAllGroups(title: title, category: category, attributes: attributes, tags: tags, offset: offset, limit:  limit);
    }
  }
  ...
  Future<List<Group>?> _loadAllGroups({String? title, String? category, Map<String, dynamic>? attributes, Set<String>? tags, GroupPrivacy? privacy, int? offset, int? limit}) async {
    if (Config().groupsUrl != null) {
