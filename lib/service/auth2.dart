...
import 'package:rokwire_plugin/ext/auth2.dart';
...
class Auth2 with Service, NetworkAuthProvider, NotificationsListener {
  ...
  void onDeepLinkUri(Uri? uri) {
    if ((uri != null) && (kIsWeb || uri.matchDeepLinkUri(Uri.tryParse(oidcRedirectUrl)))) {
    ...
    if (Config().supportsAnonymousAuth && (Config().authBaseUrl != null)) {
      ...
      if (responseJson != null) {
        ...
        if ((anonymousToken != null) && anonymousToken.isValid && (anonymousId != null) && anonymousId.isNotEmpty) {
          debugLog('anonymous auth succeeded', token: anonymousToken.refreshTokenMnemo);

          _refreshTokenFailCounts.remove(_anonymousToken?.refreshToken);
          // _clearRefreshTokenData(_anonymousToken?.refreshToken, responseToken: anonymousToken);
      ...
      debugError('anonymous auth failed', response: response);
    }
    return null;
  }
  ...
  Future<bool> processLoginResponse(Response? response, { Auth2AccountScope? scope, Auth2LoginType? loginType }) async {
    ...
    if (responseJson != null) {
      ...
      if ((token != null) && token.isValid && (account != null) && account.isValid) {
        debugLog('$authType auth succeeded', token: token.refreshTokenMnemo);
    ...
    debugError('$authType auth failed', response: response,);
    return false;
  }
  ...
  Future<void> applyLogin(Auth2Account account, Auth2Token token, { Auth2AccountScope? scope, Map<String, dynamic>? params }) async {
    ...
    // _clearRefreshTokenData(_token?.refreshToken, responseToken: token);
  ...
  void createOidcAuthenticationTimerIfNeeded() {
    if ((_oidcAuthCompleters != null) && (_processingOidcAuthentication != true)&& !kIsWeb) { //Do not start timer for the web app.
  ...
  Future<String?> getDeviceId() => kIsWeb ? Future.value('web') : RokwirePlugin.getDeviceId(deviceIdIdentifier, deviceIdIdentifier2);
  ...
  Future<void> logout({ String? reason, Auth2UserPrefs? prefs, }) async {
    Auth2Token? token = _token;
    if (token != null) {
      String reasonDescription = (reason != null) ? ' ($reason)' : '';
      debugLog('will logout$reasonDescription', token: token.refreshTokenMnemo);
      ...
      _refreshTokenFailCounts.remove(_token?.refreshToken);
      // _clearRefreshTokenData(token.refreshToken);
      ...
      debugLog('did logout $reasonDescription', token: token.refreshTokenMnemo, token2: _anonymousToken?.refreshTokenMnemo ?? '');
  ...
  Future<bool?> _logoutImpl() async {
    if ((Config().authBaseUrl != null) && (_token != null)) {
      String url = "${Config().authBaseUrl}/auth/logout";
      Map<String, String> headers = {
        'Content-Type': 'application/json'
      };
      ...
      Response? response = await Network().post(url, headers: headers, body: post, auth: Auth2Csrf());
      return (response?.statusCode == 200);
    }
    else {
      return null;
      ...
      try {
        ...
        if (refreshTokenFuture != null) {
          ...
          if (((responseCode == 400) || (!ignoreUnauthorized && responseCode == 401)) || (refreshTokenRetriesCount <= refreshTokenFailCount)) {
            ...
            debugError('failed to refresh token [$responseCode | $countsDescription : try again]', token: token.refreshTokenMnemo, response: response);
          }
        }
      }
      catch(e) {
    ...
    if ((Config().coreUrl != null) && (refreshToken != null)) {
      try {
        ...
        if (refreshTokenCompleters != null) {
          ...
          _completeRefreshToken(refreshTokenCompleters, refreshToken: refreshToken, responseToken: responseToken);
          return responseToken;
        }
      }
      catch(e) {
        debugException('e.toString()', token: token.refreshTokenMnemo);
        _clearRefreshTokenData(refreshToken, clearFailCounts: false);
      }
    }
    return null;
  }
    ...
    if ((Config().coreUrl != null) && (refreshToken != null)) {
      try {
        ...
        if ((responseJson != null) && (responseToken != null) && responseToken.isValid) {
          ...
          int refreshTokenRetriesCount = Config().refreshTokenRetriesCount;
          String refreshTokenCountsDescription = '$refreshTokenFailCount / $refreshTokenRetriesCount';

          if ((responseCode == 400) || (responseCode == 401) || (refreshTokenRetriesCount <= refreshTokenFailCount)) {
            debugError('failed to refresh token [$responseCode | $refreshTokenCountsDescription : logout]', token: token.refreshTokenMnemo, response: response);
            ...
            debugError('failed to refresh token [$responseCode | $refreshTokenCountsDescription : try again]', token: token.refreshTokenMnemo, response: response);
  ...
  void _clearRefreshTokenData(String? refreshToken, { Auth2Token? responseToken, bool clearFailCounts = true }) {
    if (refreshToken != null) {
      if (clearFailCounts) {
        _refreshTokenFailCounts.remove(refreshToken);
      }
  ...
  void _completeRefreshToken(_RefreshTokenCompleters refreshTokenCompleters, { String? refreshToken, Auth2Token? responseToken}) {
    for(_RefreshTokenCompleter completer in refreshTokenCompleters) {
      debugLog('did await refresh token', token: refreshToken?.auth2TokenMnemo, token2: responseToken?.refreshTokenMnemo ?? '');
  ...
  void debugError(String error, { String? token, Response? response }) {
    ...
    // debugPrint(log);
    FirebaseCrashlytics().recordError(log);
  }

  @protected
  void debugException(String description, { String? token }) {
    String log = 'Auth2: Exception: $description';
    if (token != null) {
      log += '; token: $token';
    }
