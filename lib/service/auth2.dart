...
class Auth2 with Service, NetworkAuthProvider, NotificationsListener implements NetworkAuthProvider {
  ...
  void onDeepLinkUri(Uri? uri) {
    if (uri != null) {
      if (!kIsWeb && !uri.matchDeepLinkUri(Uri.tryParse(oidcRedirectUrl))) {
        return;
      }
  ...
  Future<bool> verifyAccountIdentifier(String? id, String? code) async {
    if ((Config().authBaseUrl != null) && (id != null) && (code != null)) {
      String url = "${Config().authBaseUrl}/auth/identifier/verify";
  ...
  Future<Auth2Token?> refreshToken({Auth2Token? token, bool ignoreUnauthorized = false}) async {
    ...
    String futureKey = token?.refreshToken ?? '';
    if (Config().authBaseUrl != null) {
      try {
