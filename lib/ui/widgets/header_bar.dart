...
class HeaderBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  @protected
  Image? get leadingImage => (leadingAsset != null) ? Styles().uiImages?.fromString(leadingAsset!, excludeFromSemantics: true) as Image? : null;
...
class SliverToutHeaderBar extends StatelessWidget {
  ...
  Widget buildFlexibleInterior(BuildContext context) {
    Widget? image = Styles().uiImages?.fromString(flexImageUrl!, fit: BoxFit.cover, networkHeaders: Config().networkAuthHeaders, excludeFromSemantics: true);
        ...
        GestureDetector(onTap: () => onTapLeading(context), child:
          ClipOval(child:
            Container(color: leadingOvalColor, width: leadingOvalSize?.width ?? 0, height: leadingOvalSize?.height ?? 0, child:
              Styles().uiImages?.fromString(leadingAsset!, excludeFromSemantics: true)
            ),
          ),
        ),
      )
